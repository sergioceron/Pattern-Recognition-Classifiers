...
ds_23 = np.row_stack( ( np.array([e for e in ds if e[4] == 0]), np.array([e for e in ds if e[4] == 1]) ) )
loo = LeaveOneOut( len( ds_23 ) )
for train_index, test_index in loo:
    train_ds = ds_23[ train_index ]
    test_ds = ds_23[ test_index ]

    o_clase2 = np.array([e[0:4] for e in train_ds if e[4] == 0])
    o_clase3 = np.array([e[0:4] for e in train_ds if e[4] == 1])
    ...
    for rules in chain_rules:
        ...
        if clase != -1:
            break;
    print prueba, ' -> ', test_ds[0, 4], ' :: ', clase
    if test_ds[0, 4] == clase:
        correctas += 1
    print '-'*100
