...
def plot(it, clase_a, clase_b, rules):
    ...
    if rules[0][0]:  # feature 0,min
        ...
        x_min = rules[0][0]["limit"]
    else:
        x_min = 0
        x_max = 0

    patch = patches.Rectangle((float(x_min), 0), float(x_max) - float(x_min), 6,
                              hatch=hatches[int(rules[0][0]['belongs_to']) - 1], alpha=0.2, facecolor=colors[0])
    ax.add_patch(patch)

    if rules[0][1]:  # feature 0,max
        x_max = rules[0][1]["limit"]
    ...
    patch = patches.Rectangle((float(x_min), 0), float(x_max) - float(x_min), 6,
                              hatch=hatches[int(rules[0][1]['belongs_to']) - 1], alpha=0.2, facecolor=colors[0])
    ax.add_patch(patch)

    if rules[1][0]:  # feature 1,min
        y_min = rules[1][0]["limit"]
        y_max = rules[1][0]["value"]
    else:
        y_min = 0
        y_max = 0

    patch = patches.Rectangle((0, float(y_min)), 7, float(y_max) - float(y_min),
                              hatch=hatches[int(rules[1][0]['belongs_to']) - 1], alpha=0.2, facecolor=colors[1])
    ax.add_patch(patch)

    if rules[1][1]:  # feature 1,max
        y_max = rules[1][1]["limit"]
    ...
    patch = patches.Rectangle((0, float(y_min)), 7, float(y_max) - float(y_min),
                              hatch=hatches[int(rules[1][1]['belongs_to']) - 1], alpha=0.2, facecolor=colors[1])
    ax.add_patch(patch)

    ax.scatter(clase_a[:, 0], clase_a[:, 1], marker=markers[0], color='0.5', s=50, facecolors='none', edgecolors='gray',
               cmap=plt.cm.Paired)
    ax.scatter(clase_b[:, 0], clase_b[:, 1], marker=markers[1], color='0.5', s=50, facecolors='none', edgecolors='gray',
               cmap=plt.cm.Paired)
...
for train_index, test_index in loo:
    ...
    for m in range(0, NC):
        ...
        # print 'ITERACION 0'
        chain_rules = []
        rules, _clasea, _claseb = iteracion_general(expandir(o_clasea, psize), expandir(o_claseb, psize),
                                                    psize + len(px))
