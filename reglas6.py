...
ds = np.genfromtxt('./datasets/habberman.csv', delimiter=",", filling_values=0)

def expandir( datos, size=3 ):
    expandidos = []
    for patron in datos:
...
for train_index, test_index in loo:
    ...
    chain_rules = []
    a, b, rules, _clase2, _clase3 = iteracion_general( expandir(o_clase2), expandir(o_clase3), 9 )
    ...
    for it in range( 10 ):
        if len( a ) == 0 or len( b ) == 0:
            break;
        print ' ------ ITERACION ', it, ' ------- '
        a, b, rules, _clase2, _clase3 = iteracion_general( _clase2, _clase3, 9 )
        chain_rules += [ rules ]
